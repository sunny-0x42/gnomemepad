# Prepare padv6: padv5 economics + pointsv2 trade hooks (import rewritten to personal ns).
param(
...
$OUT = Join-Path $ROOT "deploy\sapphire-v6"
$rPad = Join-Path $OUT "r\$Address\gnomemepad\padv6"
...
Get-ChildItem $rPad -Filter "*.gno" | ForEach-Object {
  ...
  $c = $c -replace '(?m)^package pad\b', 'package padv6'
  # padv5-class economics (same as v5)
...
$pkgPath = "gno.land/r/$Address/gnomemepad/padv6"
...
Write-Host "Prepared Sapphire V6 (pad + pointsv2 hooks):"
Write-Host "  $pkgPath"
Write-Host "  points import: $ptsImport"
Write-Host "  bond=2 GNOT  graduate=100 GNOT  points hooks (off until SetPointsEnabled)"
Write-Host "Dir: $OUT"
