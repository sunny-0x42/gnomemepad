# Deploy padv7 (ListBuyers for holder UI). Prefer pointsv2 already live.
param(
...
& "$PSScriptRoot\prepare-sapphire-v7.ps1" -Address $Address

$padDir = Join-Path $ROOT "deploy\sapphire-v7\r\$Address\gnomemepad\padv7"
$padPath = "gno.land/r/$Address/gnomemepad/padv7"
$padv6 = "gno.land/r/$Address/gnomemepad/padv6"
...
if ($existing -match [regex]::Escape($padPath)) {
  Write-Host "padv7 already on chain, skip addpkg"
} else {
  Invoke-GnokeyDeploy -StepName "addpkg padv7" -GnokeyCmd @(
...
if (-not $SkipInit) {
  try {
    Invoke-GnokeyDeploy -StepName "Init padv7" -GnokeyCmd @(
...
if (-not $SkipHub) {
  ...
  Call-Hub @("legacy_padv6", $padv6) "hub SetModule legacy_padv6"
  Call-Hub @("pad", $padPath) "hub SetModule pad=padv7"
}

try {
  Write-Host "=== pointsv2 AllowPad padv7 ==="
  $list = [System.Collections.Generic.List[string]]::new()
  $list.AddRange([string[]]@(
  ...
  Write-Host "AllowPad: $($_.Exception.Message)"
}
...
Write-Host "OK $padPath — ListBuyers available for holder UI"
